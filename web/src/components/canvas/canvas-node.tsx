    ...
    return (
        ...
        >
            ...
            >
                ...
                <ResizeHandle corner="top-left" active={hovered || isSelected || showPanel} color={selectionBlue} fill={theme.node.panel} onMouseDown={handleResizeMouseDown} />
                <ResizeHandle corner="top-right" active={hovered || isSelected || showPanel} color={selectionBlue} fill={theme.node.panel} onMouseDown={handleResizeMouseDown} />
                <ResizeHandle corner="bottom-left" active={hovered || isSelected || showPanel} color={selectionBlue} fill={theme.node.panel} onMouseDown={handleResizeMouseDown} />
                <ResizeHandle corner="bottom-right" active={hovered || isSelected || showPanel} color={selectionBlue} fill={theme.node.panel} onMouseDown={handleResizeMouseDown} />
            </div>
            ...
            {showPanel && !isGroup && renderPanel ? <div style={{ position: "absolute", left: "50%", top: "100%", zIndex: 70, width: 600, transform: "translateX(-50%)", paddingTop: 16 }}>{renderPanel(data)}</div> : null}
        </div>
    );
});
...
function ResizeHandle({ corner, active, color, fill, onMouseDown }: { corner: ResizeCorner; active: boolean; color: string; fill: string; onMouseDown: (event: React.MouseEvent, corner: ResizeCorner) => void }) {
    const positionClass = {
    ...
    return (
        <div data-canvas-resize-handle={corner} className={`absolute z-50 grid size-7 place-items-center ${positionClass}`} onMouseDown={(event) => onMouseDown(event, corner)}>
            <span className={`pointer-events-none size-2.5 rounded-[3px] border-2 transition-opacity ${active ? "opacity-100" : "opacity-35"}`} style={{ borderColor: color, background: fill }} />
        </div>
    );
}
