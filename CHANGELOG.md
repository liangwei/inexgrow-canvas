...
+ [调整] 左侧画布面板类型筛选只显示当前画布实际存在的节点类型，类型消失时自动回到“全部”。

+ [调整] 分镜节点在画布内统一不可删除（含未生成分镜），删除键与删除按钮直接失效并提示，避免“删除后又恢复”的困惑。

+ [修复] 节点点击浮层改用内联定位，并统一画布根内 translate 工具类语义，避免宿主 Tailwind 3 同名类导致浮层/工具条偏移。

+ [修复] 画布弹层（Modal/Popover/Dropdown）滚动条统一为中性灰细条，避免亮色主题下出现系统默认黑色滚动条。

+ [调整] 宿主管理生成模式下隐藏画布底部创建类按钮后的多余分隔线，工具栏布局更紧凑。

+ [调整] 宿主管理生成模式下隐藏画布底部文本/图片/视频/音频/组创建入口及空白处创建菜单，扩展入口保留，素材统一通过主流程顶部入口添加，避免无效素材误导。

+ [修复] 宿主管理生成模式下，已验收分镜禁止连入/断开素材引用或删除；隐藏依赖宿主生成链路的上传、反推提示词、AI 超分、多角度与局部编辑入口。

+ [修复] 嵌入包发布时使用递增版本号，避免 Yarn 1 将不同提交复用为同一缓存并触发完整性校验失败。

+ [调整] 宿主管理生成时明确区分分镜策划与最终生成提示词，用户可自由编辑策划内容，并在生成前交由宿主重新编译。

+ [修复] 嵌入画布立即采用宿主项目；独立站点本地存储水合短暂覆盖项目时先保持加载态，重新校准宿主状态后再挂载子路由，避免误跳 `/canvas` 导致画布空白。

+ [修复] 节点四角增加可见缩放手柄，并优化视频设置浮层在视口边缘的尺寸、翻转与滚动表现。
+ [调整] 嵌入组件支持宿主管理生成参数，避免在导演画布中暴露未接入宿主后端的模型与通用生成设置。
+ [调整] 宿主管理生成时隐藏文本节点的独立“生图”入口，避免出现宿主未接入的生成操作。

+ [新增] 提供 `@inexgrow/canvas` 同 DOM React 嵌入组件，支持宿主项目传入画布状态、明暗主题并监听编辑结果。
+ [调整] 嵌入模式隐藏独立站点、Codex Agent 与插件管理入口，同时保留节点编辑、拖拽缩放、连接、撤销重做、导入导出和小地图等画布交互。
+ [优化] 嵌入样式限定在画布根节点内，不再修改宿主页面的 `html`、`body` 或全局明暗主题。
